"""Сервис для работы с платежами через Telegram Stars и YooKassa."""
...
async def create_yookassa_payment(
    ...
    """
    from src.services.yookassa_service import create_payment
    ...
    amount = max(1.0, amount)  # Минимум 1 рубль
    subscription_days = subscription_months * 30
    ...
    try:
        ...
        db_payment_id = Payment.create(
            user_id=user_id,
            amount_rub=amount,
            invoice_payload=f"yookassa:{user_id}:{subscription_months}:{amount}:{promo_code or ''}",
            subscription_days=subscription_days,
            promo_code=promo_code,
        ...
        logger.info(f"YooKassa payment created for user {user_id}: db_id={db_payment_id}, yookassa_id={payment_id}, amount={amount} RUB")
        ...
        logger.exception(f"Failed to create YooKassa payment for user {user_id}: {e}")
        raise
...
async def process_yookassa_payment(
    ...
    """Обрабатывает успешный платеж через YooKassa и создает пользователя в Remnawave.
    ...
    try:
        # Находим платеж в БД
        payment = Payment.get_by_yookassa_payment_id(yookassa_payment_id)
        if not payment:
            logger.error(f"Payment not found for YooKassa payment ID: {yookassa_payment_id}")
            return {"success": False, "error": "Payment not found"}
        ...
        # Парсим invoice_payload для получения subscription_months
        invoice_payload = payment.get("invoice_payload", "")
        parts = invoice_payload.split(":")
        if len(parts) >= 3 and parts[0] == "yookassa":
            subscription_months = int(parts[2])
        else:
            # Вычисляем из subscription_days
            subscription_months = subscription_days // 30
        ...
        logger.info(f"YooKassa payment processed successfully for user {user_id}: user_uuid={user_uuid}")
        ...
        logger.exception(f"Failed to process YooKassa payment {yookassa_payment_id}: {e}")
        payment = Payment.get_by_yookassa_payment_id(yookassa_payment_id)
        if payment:
            Payment.update_status(payment["id"], "failed")
        return {"success": False, "error": str(e)}
